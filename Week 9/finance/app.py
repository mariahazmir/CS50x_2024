...
def index():
    ...
    owns = db.execute(
        "SELECT symbol, SUM(shares) as shares FROM purchases GROUP BY symbol HAVING user_id = ? AND SUM(shares) > 0", user)
...
def history():
    ...
    # Summarize user's stock ownership
    owns = db.execute(
        "SELECT symbol, shares, price, time FROM purchases symbol WHERE user_id = ?", user)
...
def sell():
    ...
    if request.method == "POST":
        ...
        symbol = request.form.get("symbol")
        result = db.execute(
            "SELECT symbol FROM purchases GROUP BY symbol HAVING user_id = ? AND SUM(shares) > 0", user)
